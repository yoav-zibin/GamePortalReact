...
import './css/SocialFriends.css';
import {db, auth} from '../firebase';
import { Button } from 'reactstrap';
import firebase from 'firebase';

export default class SocialFriends extends React.Component {
    constructor(){
        super();
        this.state = {
            content : [],
    ...
    componentWillMount(){
        let self = this;
        let friendsReference = db.ref('users/'+auth.currentUser.uid+'/privateFields/friends');
        let updateUsers = (users) =>{
            this.setState({content : users});
        }
        friendsReference.on('value', function(snapshot) {
    ...
    render(){
        ...
        return(
            <div className="social-friends-inner-container">
                <ul className='social-friends-list-container'>
                    {content}
                </ul>
